...
description: Pulls the REAL artifact (prod output, live API spec, captured wire body, live query result) and diffs the code's output against it field-by-field. Use before declaring any integration "done" — unit tests prove consistency, this proves correctness. Never fabricates a sample; if the real artifact is unavailable it says so and how to get it.
