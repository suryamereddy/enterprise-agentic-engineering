---
name: done-check
description: The pre-ship gate — run BEFORE you commit or open a PR (not on every "done"). Runs an evidence bar scaled to the change size — adversarial review (1–3 reviewers by diff size), challenging coverage, E2E/automation for new scenarios, real-data proof, live-spec 1:1 check.
user-invocable: true
argument-hint: "[the change about to be committed / PR'd]"
---

# Done-check — the pre-commit / pre-PR gate

Run this **once, at the commit/PR boundary** — not on every turn, not on every "done" claim (that would be slow and waste tokens). It's the gate right before `git commit` / `gh pr create`. "It compiles and tests pass" is NOT enough. If any item is unmet, the change isn't ready — say which and why.

## 1. Adversarial review — SCALED to the change (don't always do 3)
Pick the reviewer count by diff size + risk so small changes are cheap:

| Change | Reviewers | How |
|---|---|---|
...
# then spawn the right number of @adversarial-reviewer (and your repo-specific reviewer) on the changed files;
# collect findings, dedupe, fix each, re-review the fixes.
```
A clean review on a large change is a failed review — keep digging. Address every finding, no cherry-picking.
...
Every new endpoint, flow, and failure branch gets an E2E test — incl. not-found, dependency-unavailable, 409 race, ETag/optimistic-concurrency exhaustion, upstream error → compensating action, app error, dead-letter replay, quarantine. Happy paths alone are insufficient.

## 4. Automation/integration suite covers every scenario
Not just smoke: happy, validation-negative, auth-negative, idempotency, duplicate-replay, races, load (≥20 concurrent), failure-branch proofs with datastore doc verification, dead-letter, quarantine, outbound reply verification, live-spec 1:1 parity, message offset/header checks.

## 5. Prove it with real data (use the `prod-data-verifier` agent)
...
Verify against your API spec's live/published endpoint (`<your-api-spec-url>`), not against a cached or neighboring copy. Pattern-matching from neighboring repo code is NOT verification — neighbors may be drifted.
