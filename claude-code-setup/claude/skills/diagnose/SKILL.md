---
name: diagnose
description: Root-cause a failure/bug/slowness with a deterministic feedback loop before touching the fix. Operationalizes "diagnose before you fix; never patch the symptom" as a runnable procedure. (Pattern from mattpocock/skills.)
user-invocable: true
argument-hint: "[the failing test / bug / slow step]"
---
...
- **HTTP/API:** `curl` the endpoint directly and capture the exact request/response. If logs are behind a corp proxy, use a token + a direct `curl` to the log API instead of an SDK that breaks on the proxy cert.
...
- field-by-field payload diff (your `verify-prod` skill)
- old vs new model deserialization of the same stored doc
- prod baseline vs your output (your `verify-prod`)
