---
description: "Verify code output against real production/live data — pull the real artifact and diff field-by-field; never fabricate a sample"
agent: "agent"
tools: ["search/codebase", "search", "search/usages", "execute/runInTerminal", "execute/runTests"]
---
...
|---|---|---|
| API payload shape | live OpenAPI spec | fetch the spec endpoint and extract to `/tmp/api-spec.json` |
...
- Resources behind an IP allowlist or private endpoint may need VPN; off-VPN, fall back to a direct token-based request (`curl` with a scoped bearer token) instead of an SDK that breaks on a corporate-proxy cert.
- For the deeper version, hand off to the `prod-data-verify` prompt.
