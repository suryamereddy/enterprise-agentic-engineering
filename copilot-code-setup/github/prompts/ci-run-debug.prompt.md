...
agent: "agent"
tools: ["execute/runInTerminal", "search/codebase", "search"]
---

# Debug a CI run — ${input:target:workflow or branch}

```bash
# gh auth switch --user <your-gh-account>   # uncomment if the repo needs a specific account
...
- **`--no-build` "argument …dll invalid"** (.NET 10 + xunit 3.x) → drop `--no-build`.
...
> Building blocks: `gh run view --log-failed` pulls only failing steps. Pairs with the `pr-review-triage` prompt.
